...
namespace OpenETaxBill.Channel.Library.Security.Signature
{
    public class SeedCs
    {
        ...
        //
        //-------------------------------------------------------------------------------------------------------------------------//
        private readonly static Lazy<SeedCs> m_seedCs = new Lazy<SeedCs>(() =>
        {
            return new SeedCs();
        });

        /// <summary></summary>
        public static SeedCs SNG
        {
            get
            {
                return m_seedCs.Value;
            }
        }
        ...
        public byte[] IV
        {
            ...
            {
                if (_iv == null)
                    _iv = Encoding.Default.GetBytes("0123456789012345");
        ...
        // external functions
        //-------------------------------------------------------------------------------------------------------------------------//
        public byte[] Encrypt(byte[] p_data, byte[] p_seedKey)
        {
            return Encrypt(p_data, p_seedKey, false);
        }

        public byte[] Encrypt(byte[] p_data, byte[] p_seedKey, bool p_cbcPad)
        {
            int _insize = p_data.Length;

            int _outsize;            
            if (p_cbcPad == true)
                _outsize = (int)((p_data.Length + 16) / 16) * 16;
            else
                _outsize = (int)((p_data.Length) / 16) * 16;

            byte[] _result = new byte[_outsize];
            Buffer.BlockCopy(p_data, 0, _result, 0, _insize);

            if (p_cbcPad == true)
            {
                int _padding = _outsize - _insize;
                if (_padding == 0)
                    _padding = 16;

                for (int i = _insize; i < _outsize; i++)
                    _result[i] = (byte)_padding;
            }

            uint[] _roundKey = new uint[32];
            SeedEncRoundKey(ref _roundKey, p_seedKey);

            // CBC 모드 초기값 IV
            byte[] _prevData = new byte[16];
            for (int i = 0; i < _outsize; i += 16)
            {
                byte[] _dataBlock = new byte[16];
                
                Buffer.BlockCopy(_result, i, _dataBlock, 0, 16);
                if (p_cbcPad == true)
                    XorByte(ref _dataBlock, _prevData);

                SeedEncryptBlock(ref _dataBlock, _roundKey);

                Buffer.BlockCopy(_dataBlock, 0, _result, i, 16);
                if (p_cbcPad == true)
                    Buffer.BlockCopy(_dataBlock, 0, _prevData, 0, 16);
            }

            return _result;
        }

        public byte[] Decrypt(byte[] p_data, byte[] p_seedKey)
        {
            return Decrypt(p_data, p_seedKey, false, null);
        }

        public byte[] Decrypt(byte[] p_data, byte[] p_seedKey, bool p_cbcPad, byte[] p_byteIV)
        {
            byte[] _result = new byte[p_data.Length];

            int _outsize = p_data.Length;
            uint[] _roundKey = new uint[32];
            SeedEncRoundKey(ref _roundKey, p_seedKey);
            ...
            if (p_byteIV != null)
                ...
                _prevData = new byte[16];

            for (int i = 0; i < _outsize; i += 16)
            {
                byte[] _dataBlock = new byte[16];
                Buffer.BlockCopy(p_data, i, _dataBlock, 0, 16);

                SeedDecryptBlock(ref _dataBlock, _roundKey);
                if (p_cbcPad == true)
                    XorByte(ref _dataBlock, _prevData);

                Buffer.BlockCopy(_dataBlock, 0, _result, i, 16);
                if (p_cbcPad == true)
                    Buffer.BlockCopy(p_data, i, _prevData, 0, 16);
            }

            if (p_cbcPad == true)
            {
                int _padding = _result[_outsize - 1];
                if (_padding > 0 && _padding <= 16)
                {
                    int i = 0;
                    for (i = _outsize - 1; i > _outsize - _padding; i--)
                    {
                        if (_result[i] != _padding)
                            break;
                    }

                    if (i == _outsize - _padding)
                    {
                        _outsize -= _padding;
                        Buffer.BlockCopy(_result, 0, p_data, 0, _outsize);
                    
                        _result = new byte[_outsize];
                        Buffer.BlockCopy(p_data, 0, _result, 0, _outsize);
                    }
                }
            }
