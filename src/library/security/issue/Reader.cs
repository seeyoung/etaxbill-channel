...
namespace OpenETaxBill.Channel.Library.Security.Issue
{
    public class Reader
    {
        ...
        /// </summary>
        /// <param name="p_invoiceXml">전자세금계산서</param>
        public Reader(XmlDocument p_invoiceXml)
        {
            this.InvoiceXml = p_invoiceXml;
        }
        ...
        // <ExchangedDocument>
        //-------------------------------------------------------------------------------------------------------------------------
        private void AssignFieldValue(DataRow p_invoiceRow, string p_name, string p_value, string p_format)
        {
            if (p_invoiceRow.Table.Columns.IndexOf(p_name) >= 0)
            {
                if (p_invoiceRow.Table.Columns[p_name].DataType == typeof(DateTime))
                {
                    p_invoiceRow[p_name] = DateTime.ParseExact(p_value, p_format, System.Globalization.CultureInfo.CurrentCulture);
                }
                else if (p_invoiceRow.Table.Columns[p_name].DataType == typeof(Decimal))
                {
                    if (String.IsNullOrEmpty(p_value) == false)
                        p_invoiceRow[p_name] = Decimal.Parse(p_value);
                }
                else
                {
                    p_invoiceRow[p_name] = p_value;
                }
            }
        }

        private void ExchangedDocument(DataRow p_invoiceRow)
        {
            ...
            if (_nav.MoveToChild(XPathNodeType.Element) == true)
            {
                do
                {
                    ...
                    this.AssignFieldValue(p_invoiceRow, _name, _nav.Value, _format);
                }
        ...
        //-------------------------------------------------------------------------------------------------------------------------
        private void TaxInvoiceDocument(DataRow p_invoiceRow)
        {
                            ...
                            while (_iter1.MoveNext() == true)
                            {
                                ...
                                else if (_name == "AcceptablePeriod" && _iter1.Current.HasChildren == true)
                                {
                                    ...
                                    while (_iter2.MoveNext() == true)
                                    {
                                        ...
                                        this.AssignFieldValue(p_invoiceRow, _name, _iter2.Current.Value, _format);
                                    }

                                    _name = "";
                                }

                                this.AssignFieldValue(p_invoiceRow, _name, _iter1.Current.Value, _format);
                            }
        ...
        //-------------------------------------------------------------------------------------------------------------------------
        private void TaxInvoiceTradeSettlement(DataRow p_invoiceRow)
        {
            ...
            if (_nav.MoveToChild(XPathNodeType.Element) == true)
            {
                do
                {
                    ...
                    else if (_name == "SpecifiedPaymentMeans" && _nav.HasChildren == true)
                    {
                        ...
                        this.AssignFieldValue(p_invoiceRow, _name, _paidAmount, _format);
                        _name = "";
                    }
        ...
        private void TaxInvoiceTradeLineItem(DataTable p_lineitemTbl, string p_issue_id)
        {
            ...
            do
            {
                ...
                {
                    DataRow _lineitemRow = p_lineitemTbl.NewRow();

                    foreach (DataColumn _dc in p_lineitemTbl.Columns)
                    {
                    ...
                    _lineitemRow["issueId"] = p_issue_id;
                    ...
                    while (_iter1.MoveNext() == true)
                    {
                        ...
                        string _name = _iter1.Current.Name;
                        {
                    ...
                    p_lineitemTbl.Rows.Add(_lineitemRow);
                }
        ...
        public DataSet TaxInvoice()
        {
            ...
            string _issueId = Convert.ToString(_invoiceRow["issueId"]);
        ...
        public DataSet TaxPurchase()
        {
            ...
            string _issueId = Convert.ToString(_purchaseRow["issueId"]);
