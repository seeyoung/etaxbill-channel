...
namespace OpenETaxBill.Channel.Library.Security.Issue
{
    public class Validator
    {
        ...
        /// </summary>
        /// <param name="xmlFileName">검증할 XML 파일 경로</param>
        public string DoValidation(string p_xmlString)
        {
            return this.DoValidation(
                new MemoryStream(Encoding.UTF8.GetBytes(p_xmlString))
                );
        }

        public string DoValidation(Stream p_stream)
        {
            var _result = "";
            ...
            try
            {
                ...
                XmlReader _reader = XmlReader.Create(p_stream, this.ReaderSettings);
                while (_reader.Read())
                    ;
            }
            catch (Exception ex)
        ...
        //
        //-------------------------------------------------------------------------------------------------------------------------
        public int CheckInvoiceDataTable(DataSet p_invoiceDS)
        {
            return this.CheckInvoiceDataTable(
                    p_invoiceDS.Tables["TB_eTAX_INVOICE"], p_invoiceDS.Tables["TB_eTAX_LINEITEM"]
                );
        }

        public int CheckInvoiceDataTable(DataTable p_invoiceTbl, DataTable p_lineitemTbl)
        {
            int _result = 0;

            foreach (DataRow _dr in p_invoiceTbl.Rows)
            {
                string _filterExpression = String.Format("issueId='{0}'", _dr["issueId"]);

                string _rowError = this.CheckInvoiceDataTable(_dr, p_lineitemTbl.Select(_filterExpression));
        ...
        public string CheckInvoiceDataTable(DataRow p_invoiceRow, DataRow[] p_lineitemRows)
        {
            var _result = "";

            try
            {
                string _issueId = Convert.ToString(p_invoiceRow["issueId"]);
                if (String.IsNullOrEmpty(_issueId) == false)
                {
                    DateTime _issueDate = Convert.ToDateTime(p_invoiceRow["issueDate"]);
                ...
                string _typeCode = Convert.ToString(p_invoiceRow["typeCode"]);
                if (String.IsNullOrEmpty(_typeCode) == false)
                {
                    if (_typeCode.Substring(0, 2) == "02" || _typeCode.Substring(0, 2) == "04")
                    {
                        if (String.IsNullOrEmpty(Convert.ToString(p_invoiceRow["amendmentCode"])) == true)
                            _result += "INV110: 수정세금계산서 또는 수정계산서인 경우에는 '수정코드(amendmentCode)'에 값이 있어야 합니다." + "\n";
                    }
                    else
                    {
                        if (String.IsNullOrEmpty(Convert.ToString(p_invoiceRow["amendmentCode"])) == false)
                    ...
                    {
                        if (String.IsNullOrEmpty(Convert.ToString(p_invoiceRow["purposeCode"])) == true)
                ...
                string _description = Convert.ToString(p_invoiceRow["description"]);

                string _invoicerId = Convert.ToString(p_invoiceRow["invoicerId"]);
                if (String.IsNullOrEmpty(_invoicerId) == false)
                {
                    if (_invoicerId.Length != 10)
                        _result += "INV120: '공급사업자(invoicerId)'는 사업자번호인 10자리만 가능 합니다." + "\n";

                    if (String.IsNullOrEmpty(Convert.ToString(p_invoiceRow["invoicerName"])) == true)
                        _result += "INV122: '공급사업자명(invoicerName)'에 값이 있어애 합니다." + "\n";

                    if (String.IsNullOrEmpty(Convert.ToString(p_invoiceRow["invoicerPerson"])) == true)
                ...
                string _invoiceeId = Convert.ToString(p_invoiceRow["invoiceeId"]);
                if (String.IsNullOrEmpty(_invoiceeId) == false)
                {
                    ...
                    if (String.IsNullOrEmpty(Convert.ToString(p_invoiceRow["invoiceeName"])) == true)
                        _result += "INV132: '공급받는자명(invoiceeName)'에 값이 있어야 합니다." + "\n";

                    if (String.IsNullOrEmpty(Convert.ToString(p_invoiceRow["invoiceePerson"])) == true)
                        _result += "INV134: '공급받는대표자명(invoiceePerson)'에 값이 있어야 합니다." + "\n";

                    string _invoiceeKind = Convert.ToString(p_invoiceRow["invoiceeKind"]);
                    ...
                    string _invoiceeEMail1 = Convert.ToString(p_invoiceRow["invoiceeEMail1"]);
                ...
                string _brokerId = Convert.ToString(p_invoiceRow["brokerId"]);
                    ...
                    _typeCode == "0105" || _typeCode == "0205" || _typeCode == "0305" || _typeCode == "0405")
                {
                    ...
                    if (String.IsNullOrEmpty(Convert.ToString(p_invoiceRow["brokerName"])) == true)
                        _result += "INV154: '수탁사업자명(brokerName)'에 값이 있어야 합니다." + "\n";

                    if (String.IsNullOrEmpty(Convert.ToString(p_invoiceRow["brokerPerson"])) == true)
                ...
                Decimal _chargeTotal = Convert.ToDecimal(p_invoiceRow["chargeTotal"]);
                Decimal _taxTotal = Convert.ToDecimal(p_invoiceRow["taxTotal"]);
                Decimal _grandTotal = Convert.ToDecimal(p_invoiceRow["grandTotal"]);
                if (_grandTotal == (_chargeTotal + _taxTotal))
                {
                    ...
                    Decimal _paidCash = Convert.ToDecimal(p_invoiceRow["paidCash"]);
                    Decimal _paidCheck = Convert.ToDecimal(p_invoiceRow["paidCheck"]);
                    Decimal _paidNote = Convert.ToDecimal(p_invoiceRow["paidNote"]);
                    Decimal _paidCredit = Convert.ToDecimal(p_invoiceRow["paidCredit"]);
                ...
                if (p_lineitemRows.Length > 0)
                {
                    ...
                    foreach (DataRow _dr in p_lineitemRows)
                    {
        ...
        public NameValueCollection GetCollectionFromResult(string p_errorMessage)
        {
            NameValueCollection _result = new NameValueCollection();

            string[] _lines = p_errorMessage.Split('\n');
        ...
        ///// </summary>
        ///// <param name="p_customerid">사업자번호 또는 주민번호</param>
        ///// <returns></returns>
        //public bool CheckCustomerId(string p_customerid)
        //{
        //    var _result = false;

        //    string _customerid = p_customerid.Trim();
        ...
        /// 주민번호 검증 함수 
        /// </summary>
        /// <param name="p_socialNo">주민번호</param>
        /// <returns></returns>
        public bool VerifySSN(string p_socialNo)
        {
            var _result = true;

            string _socialNo = p_socialNo.Trim();
            ...
            while (_socialNo != "9999999999999")
            {
                ...
                string _birthYear = ('2' >= _socialNo[6]) ? "19" : "20";

                // 연도 두 자리를 추출하여 추가합니다.
                _birthYear += _socialNo.Substring(0, 2);

                // 월 단위 두 자리를 추출합니다.
                string _birthMonth = _socialNo.Substring(2, 2);

                // 일 단위 두 자리를 추출합니다.
                string _birthDate = _socialNo.Substring(4, 2);

                try
                {
                    // 정수로 변환을 시도합니다. 예외가 생기면 catch 블럭으로 이동됩니다.
                    int _bYear = int.Parse(_birthYear);
                    int _bMonth = int.Parse(_birthMonth);
                    int _bDate = int.Parse(_birthDate);
        ...
        /// 사업자등록번호 검증 함수
        /// </summary>
        /// <param name="p_businessNo">사업자번호</param>
        /// <returns></returns>
        public bool VerifyBSN(string p_businessNo)
        {
            var _result = false;
            ...
            try
            {
                ...
                _checker += int.Parse(p_businessNo.Substring(0, 1)) * 1;
                _checker += int.Parse(p_businessNo.Substring(1, 1)) * 3;
                _checker += int.Parse(p_businessNo.Substring(2, 1)) * 7;
                _checker += int.Parse(p_businessNo.Substring(3, 1)) * 1;
                _checker += int.Parse(p_businessNo.Substring(4, 1)) * 3;
                _checker += int.Parse(p_businessNo.Substring(5, 1)) * 7;
                _checker += int.Parse(p_businessNo.Substring(6, 1)) * 1;
                _checker += int.Parse(p_businessNo.Substring(7, 1)) * 3;
                _checker += int.Parse(p_businessNo.Substring(8, 1)) * 5 / 10;
                _checker += int.Parse(p_businessNo.Substring(8, 1)) * 5 % 10;
                _checker += int.Parse(p_businessNo.Substring(9, 1));
