...
namespace OpenETaxBill.Channel.Library.Security.Issue
{
    public class Writer
    {
        ...
        /// 전자세금계산서 1매를 생성한다.
        /// </summary>
        /// <param name="p_exchangeSet">전자세금계산 신고 항목 데이터셋(0-Invoice,1-LineItem)</param>
        public Writer(DataSet p_exchangeSet)
        {
            ExchangeSet = p_exchangeSet;
        ...
        {
            return TaxDocument.CreateElement(p_qualifiedName, TaxNamespaces.DefaultNamespace);
        }
        ...
        private void InvoiceTextElement(XmlElement p_element, string p_qualifiedName, string p_fieldName)
        {
            TextElement(InvoiceRow, p_element, p_qualifiedName, p_fieldName);
        }

        private void InvoiceDateElement(XmlElement p_element, string p_qualifiedName, string p_fieldName, string p_format)
        {
            DateElement(InvoiceRow, p_element, p_qualifiedName, p_fieldName, p_format);
        }

        private string InvoiceDateToString(string p_fieldName, string p_format)
        {
            return DateToString(InvoiceRow, p_fieldName, p_format);
        }
        
        private bool IsInvoiceZeroOrEmpty(string p_fieldName)
        {
            return IsZeroOrEmpty(InvoiceRow, p_fieldName);
        }

        private void TextElement(DataRow p_datrow, XmlElement p_element, string p_qualifiedName, string p_fieldName)
        {
            if (p_datrow[p_fieldName] != DBNull.Value)
            {
                string _fieldValue = Convert.ToString(p_datrow[p_fieldName]);
                InsertElement(p_element, p_qualifiedName, _fieldValue);
            }
        }

        private void DateElement(DataRow p_datrow, XmlElement p_element, string p_qualifiedName, string p_fieldName, string p_format)
        {
            if (p_datrow[p_fieldName] != DBNull.Value)
            {
                string _fieldValue = DateToString(p_datrow, p_fieldName, p_format);
                InsertElement(p_element, p_qualifiedName, _fieldValue);
            }
        }

        private string DateToString(DataRow p_datrow, string p_fieldName, string p_format)
        {
            //if (p_datrow[p_fieldName] == DBNull.Value)
            //    throw new ProxyException();

            return Convert.ToDateTime(p_datrow[p_fieldName]).ToString(p_format);
        }

        private bool IsZeroOrEmpty(DataRow p_datrow, string p_fieldName)
        {
            var _result = true;

            if (p_datrow[p_fieldName] != DBNull.Value)
            {
                if (Convert.ToDecimal(p_datrow[p_fieldName]) != 0)
                    _result = false;
            }
                ...
                XmlElement _invoicee = CreateElement("InvoiceeParty");
                {
                    ...
                    if (_invoicee.HasChildNodes == true)
                    {
                        ...
                        XmlElement _specifiedOrg = CreateElement("SpecifiedOrganization");
                        {
                            ...
                            InvoiceTextElement(_specifiedOrg, "TaxRegistrationID", "invocieeOrgId");
        ...
        private XmlElement TaxInvoiceTradeLineItem(DataRow p_itemrow)
        {
            XmlElement _lineitem = CreateElement("TaxInvoiceTradeLineItem");
            {
                // 물품 일련번호 (0 or 1) 
                TextElement(p_itemrow, _lineitem, "SequenceNumeric", "seqNo");

                // 물품과 관련된 자유기술문 (0 or 1) 
                TextElement(p_itemrow, _lineitem, "DescriptionText", "description");

                // 물품 공급 가액 (0 or 1) 
                TextElement(p_itemrow, _lineitem, "InvoiceAmount", "invoiceAmount");

                // 물품 수량 (0 or 1) 
                TextElement(p_itemrow, _lineitem, "ChargeableUnitQuantity", "quantity");

                // 물품에 대한 규격 (0 or 1) 
                TextElement(p_itemrow, _lineitem, "InformationText", "information");

                // 물품명 (0 or 1) 
                TextElement(p_itemrow, _lineitem, "NameText", "itemName");

                // 물품 공급일자 (0 or 1) 
                DateElement(p_itemrow, _lineitem, "PurchaseExpiryDateTime", "purchaseDate", "yyyyMMdd");

                // 물품 세액 (0 or 1) 
                XmlElement _totalTax = CreateElement("TotalTax");
                {
                    TextElement(p_itemrow, _totalTax, "CalculatedAmount", "taxAmount");
                ...
                {
                    TextElement(p_itemrow, _unitPrice, "UnitAmount", "unitPrice");
        ...
        private XmlElement TaxInvoice()
        {
            XmlElement _taxInvoice = CreateElement("TaxInvoice");
            {
                ...
                foreach (string _key in TaxNameCollections.AllKeys)
                {
                    if (_key.IndexOf(':') < 1)
                    {
                        string _name = (String.IsNullOrEmpty(_key) == true) ? "xmlns" : "xmlns:" + _key;
