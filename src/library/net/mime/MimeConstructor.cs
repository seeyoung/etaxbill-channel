...
namespace OpenETaxBill.Channel.Library.Net.Mime
{
    /// <summary>
    /// Mime constructor.
    /// </summary>
    public class MimeConstructor
    {
        ...
        {
            MemoryStream _mime = new MemoryStream();
            byte[] _buffer = null;

            string _mainBoundary = "----=_NextPart_" + Guid.NewGuid().ToString().Replace("-", "_");
            {
                // Message-ID:
                ...
                // To:
                _buffer = Encoding.UTF8.GetBytes("To: " + this.ConstructAddress(m_to));
                _mime.Write(_buffer, 0, _buffer.Length);
            ...
            string _bodyBoundary = "----=_NextPart_" + Guid.NewGuid().ToString().Replace("-", "_");
            {
