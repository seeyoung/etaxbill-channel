...
namespace OpenETaxBill.Channel.Library.Net
{
    ...
    public class NetCore
    {
        ...
        public byte[] ReadData(Socket socket, string terminator, string removeFromEnd)
        {
            ...
            ReadReplyCode _replyCode = ReadData(socket, out _storeStream, null, 10000000, 300000, terminator, removeFromEnd);
        ...
        {
            ReadReplyCode _replyCode = ReadReplyCode.Ok;
        ...
        public void SendData(Socket p_socket, string p_sendData)
        {
            byte[] _dataBytes = Encoding.UTF8.GetBytes(p_sendData.ToCharArray());

            int _sentCount = p_socket.Send(_dataBytes, _dataBytes.Length, 0);
        ...
        /// <param name="IP"></param>
        /// <returns></returns>
        public string GetHostName(string p_ip_address)
        {
            string _result = "UnkownHost";

            try
            {
                IPHostEntry _entry = System.Net.Dns.GetHostEntry(p_ip_address);
            ...
            while (_byte > -1)
            {
                ...
                if (_byte == '=')
                {
                    ...
                    if (_buffer[0] != '\r' || _buffer[1] != '\n')
                    {
                        try
                        {
                            int _value = int.Parse(Encoding.UTF8.GetString(_buffer), System.Globalization.NumberStyles.HexNumber);
                            string _encodedChar = p_encoding.GetString(new byte[] { (byte)_value });
                ...
                {
                    string _encodedChar = p_encoding.GetString(new byte[] { (byte)_byte });
